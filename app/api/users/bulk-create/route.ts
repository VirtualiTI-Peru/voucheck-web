...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!orgId || users.length === 0) {
      return NextResponse.json({ error: 'Missing orgId or users' }, { status: 400 });
    }
    ...
    for (let i = 0; i < users.length; i += 1) {
      ...
      if (!password || password.length < 6) {
        results.push({
          ...
          success: false,
          emailSent: false,
          error: 'Password inválido (mínimo 6 caracteres).',
        });
        continue;
      }
      ...
      if (createError || !created?.user) {
        ...
        results.push({
          ...
          success: false,
          emailSent: false,
          error: alreadyRegistered ? 'El usuario ya existe.' : createError?.message || 'No se pudo crear el usuario.',
        });
        continue;
      }
    ...
    return NextResponse.json({ error: error?.message || 'Failed to bulk create users' }, { status: 500 });
  }
}
